    ...
    const handleSignup = async (e) => {
        ...
        try {
            const { error: authError } = await authClient.signUp.email({
                ...
                email,
                password,
                image: photoUrl, // Better-auth uses 'image' for photoURL
                role,
            });
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#050B14] px-4 py-12 font-sans">
            <div className="relative w-full max-w-md overflow-hidden rounded-3xl p-[2px]">
                ...
                <div className="relative w-full h-full bg-[#0B1120] rounded-[22px] p-8 z-10 flex flex-col">
                    ...
                    {error && <div className="mb-6 p-3 text-sm text-red-400 bg-red-400/10 border border-red-400/20 rounded-xl text-center">{error}</div>}
                    {success && <div className="mb-6 p-3 text-sm text-green-400 bg-green-400/10 border border-green-400/20 rounded-xl text-center">{success}</div>}
                    ...
                    <form onSubmit={handleSignup} className="space-y-4">
                        ...
                        <div className="input-wrapper">
                            ...
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Full Name" className="w-full bg-transparent border-none outline-none text-white text-sm px-3 py-3" />
                        </div>
                        {/* Email Input */}
                        <div className="input-wrapper">
                        ...
                        <div className="input-wrapper">
                            ...
                            <input type="text" value={photoUrl} onChange={(e) => setPhotoUrl(e.target.value)} required placeholder="Photo URL" className="w-full bg-transparent border-none outline-none text-white text-sm px-3 py-3" />
                        </div>

                        {/* Role selection */}
                        <div className="flex flex-col space-y-3 mb-6">
                            <Label className="text-sm font-medium text-gray-400 pl-1">Choose Role</Label>
                        ...
                        <button type="submit" disabled={isLoading} className="w-full py-3.5 bg-[#8B5CF6] hover:bg-[#7C3AED] text-white text-sm font-semibold rounded-xl transition-all disabled:opacity-50 mt-4">
                            {isLoading ? "Creating..." : "Create Account"}
                        </button>
                    </form>

                    <p className="mt-8 text-center text-sm text-gray-400">
                        Already have an account? <Link href={`/auth/signin?redirect=${redirectTo}`} className="text-[#8B5CF6] font-medium hover:underline">Sign In</Link>
                    </p>
                </div>
            </div>
        </div>
